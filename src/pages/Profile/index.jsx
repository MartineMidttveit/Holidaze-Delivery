...
const Profile = () => {
  ...
  useEffect(() => {
    const title = `${username}'s Profile | Holidaze`;
    document.title = title;

    const description = `View ${username}'s profile, including their venues, bookings, and favorites on Holidaze.`;
        ...
        return userBookings
          ?.filter(
            booking =>
              booking.venue &&
              booking.dateFrom &&
              !isPast(new Date(booking.dateTo)) // Include bookings happening today or in the future
          )
  ...
  const renderLabel = () => {
    switch (selectedOption) {
      case 'bookings':
        return 'Upcoming Bookings'; // Updated for clarity
      case 'favorites':
        return 'Favorites';
      case 'past-stays':
        return 'Past Stays';
